...
Vector DvlFactor::evaluateError(const Pose3 &pose, const Vector3 &estimated_velocity, const imuBias::ConstantBias &bias_gyro,
    ...
    // Vector3  vel_world_dvl = base_link_dvl_rotations_.matrix().transpose()*(R_world_base_link.transpose().matrix()*estimated_velocity);
    // // angular velocity contribution
    // std::cout<<vel_world_dvl<<std::endl;
    ...
    if(H3){
        ...
        // H3_mat.block<3,3>(0,3) = skewSymmetric(base_to_dvl_offset_);
        *H3 = H3_mat;
    }
  return (Vector(error));
}
